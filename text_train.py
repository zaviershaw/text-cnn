...
def train():
    ...
    writer_val = tf.summary.FileWriter(tensorboard_dir+"/val")
    ...
    for epoch in range(config.num_epochs):
        ...
        for x_batch, y_batch in batch_train:
            ...
            if global_step % config.print_per_batch == 0:
                ...
                with open(tensorboard_dir+'/log-cnn.csv', 'a+') as f:
                    f.write(str(epoch) + " " + str(global_step) + " " + str(train_loss) + " " + str(train_accuracy) + " " + str(val_loss) + 
                        " " + str(val_accuracy) + '\n')
                start = time.time()
